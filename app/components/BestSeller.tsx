...
const BestSeller = () => {
  return (
    <main className='bg-[#f6f1e5] mb-8'>
        ...
        <div id='menu' className="product-row grid grid-cols-2 md:grid-cols-4 gap-8 pt-8 pb-8 justify-between">
            {data.products.map((product) => (
                <div className="card rounded-md" key={product.id}>
                    <Image src={product.imageUrl} alt={product.name} height={200} width={200} className='w-full border-none' />
                    <div className="productInfo flex-grow pt-6 border-r border-l border-b rounded-b-md">
                        <div className="px-4">  
                            <h6 className='cursor-pointer text-coffee text-base md:text-xl font-semibold'>{product.name}</h6>

                            <div className="flex justify-between text-coffee py-2 md:py-6 text-xl font-semibold md:leading-2.5 items-center">
                                    <p>${product.price}</p>
                        ...
                        <Button className="select text-center custom-button w-full mt-3 md:mt-6 rounded-none">
                            <h1 className='text-base lg:text-lg leading-3.5 tracking-wider font-normal'>SELECT OPTION</h1>
                        </Button>
                    </div>
                </div>
            ))}
